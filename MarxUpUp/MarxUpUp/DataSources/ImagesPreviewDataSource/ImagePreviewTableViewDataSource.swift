...
class ImagePreviewTableViewDataSource: NSObject {
    ...
    init(withImagesFilteredBy filter: DataFilter, _ requester: ImageDataRequester = ImageDataRequester(),
         _ cache: NSCache<NSString, UIImage> = NSCache<NSString, UIImage>(),
         _ databaseManager: LocalContentManaging = DatabaseManager()) {
    ...
    private func set(_ image: UIImage, toCell cell: ImagesPreviewTableViewCell,
                     fromTableView table: UITableView, atIndexPath indexPath: IndexPath) {
...
extension ImagePreviewTableViewDataSource {
    private func loadImage(forCell cell: ImagesPreviewTableViewCell,
                           withIndexPath indexPath: IndexPath,
                           fromTableView table: UITableView) {
        ...
        let imageDataHandler: (Data?) -> Void = { data in
            guard let unwrappedData = data,
                ...
                indexPath.row < self.imageURLs.count else {
