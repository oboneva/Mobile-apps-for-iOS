...
class ToolboxInitializer: NSObject {
    ...
    static func addToolboxItems(toView scrollView: UIStackView?,
                                withTarget targetController: ToolboxStackController,
                                _ selector: Selector, andDataSource dataSource: ToolboxDataSource) {
        guard let view = scrollView else {
            print("Error: Source view is nil")
            return
        }

        _ = Array(0..<dataSource.itemsCount).map {
