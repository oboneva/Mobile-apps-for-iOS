...
class NetworkResponseParserTests: XCTestCase {
    ...
    func testReturnedValueDoesNotContainAnimatedPictures() {
        ...
        waitForExpectations(timeout: 1) { error -> Void in if error != nil {
            XCTFail("Error: No response from the parser.") } }
    }
    ...
    func testPagination() {
        ...
        parser.linksFromJSONDict(dict as [String : AnyObject], countPerPage: 2) { (_) in
            expectation.fulfill()
        }
        waitForExpectations(timeout: 1) { error -> Void in if error != nil {
            XCTFail("Error: No response from the parser.") } }
