...
class DocsPreviewControllerTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        controller = Storyboard.main.viewController(fromClass: DocsPreviewViewController.self)
            as DocsPreviewViewController
...
class MockDocsPreviewTableViewDataSource: DocsPreviewTableViewDataSource {
    var doc = PDFDocument(url: URL(fileURLWithPath: Bundle.main.path(forResource: "test-document",
                                                                     ofType: "pdf") ?? ""))
