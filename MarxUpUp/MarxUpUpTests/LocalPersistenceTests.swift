...
class LocalPersistenceTests: XCTestCase {
    ...
    func populate() {
        ...
        func insertImage(_ data: Data) -> Image? {
            let image = NSEntityDescription.insertNewObject(forEntityName: "Image", into: fakePersistenceContainer.viewContext)
            image.setValue(data, forKey: "imageData")
            return image as? Image
        }
        
        func insertPDF(_ data: Data) -> PDF? {
            let pdf = NSEntityDescription.insertNewObject(forEntityName: "PDF", into: fakePersistenceContainer.viewContext)
            pdf.setValue(data, forKey: "pdfData")
            return pdf as? PDF
        }
        
        let _ = insertPDF(documentData)
        let _ = insertImage(imageData)
    ...
    func clearData() {
        let imagesRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest<NSFetchRequestResult>(entityName: "Image")
        let pdfRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest<NSFetchRequestResult>(entityName: "PDF")
